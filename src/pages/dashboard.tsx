...
function Dashboard() {
  ...
  const stats = useMemo(() => {
    ...
    for (const invoice of invoices) {
      ...
      if (invoice.isSent && !isCancelled) {
        ...
        if (invoice.sentAt) {
          ...
          if (sentDate.getFullYear() === thisYear) {
            monthlyCounts[sentDate.getMonth()]++;
          }
        }
      }
        ...
        if (invoice.paidAt) {
          ...
          if (invoice.sentAt) {
            const days = Math.floor(
              (paidDate.getTime() - new Date(invoice.sentAt).getTime()) / 86_400_000,
            );
    ...
    const monthTrend =
      receivedLastMonth > 0
        ? Math.round(((receivedThisMonth - receivedLastMonth) / receivedLastMonth) * 100)
        : null;
    ...
    <AppShell>
      <div className="p-6">
        <div className="mb-6 flex items-center justify-between">
          <div>
            <h1 className="text-xl font-medium text-[#0F1B3D]">Dashboard</h1>
        ...
        <div className="mb-3 grid grid-cols-1 gap-3 sm:grid-cols-3">
          ...
          <div className="rounded-xl border border-[#EFEAE0] bg-white p-4">
            <p className="text-xs text-[#8A93AC]">Received this month</p>
            <p className="mt-1 text-xl font-medium text-[#0F1B3D]">
              {loading ? "—" : `₦${stats.receivedThisMonth.toLocaleString()}`}
            </p>
          ...
          <div className="rounded-xl border border-[#EFEAE0] bg-white p-4">
            <p className="text-xs text-[#8A93AC]">Overdue</p>
            <p
              className={`mt-1 text-xl font-medium ${
                stats.overdueCount > 0 ? "text-[#C4432E]" : "text-[#0F1B3D]"
              }`}
            >
            ...
            {!loading && stats.overdueCount > 0 && (
              <p className="mt-1 text-[11px] text-[#8A93AC]">
                Oldest: {stats.oldestOverdueDays} day{stats.oldestOverdueDays === 1 ? "" : "s"}
              </p>
            )}
          </div>
        </div>
        ...
        <div className="mb-3 grid grid-cols-1 gap-3 md:grid-cols-[180px_1fr]">
          <div className="flex flex-col items-center justify-center rounded-xl border border-[#EFEAE0] bg-white p-4">
            ...
            <p className="mt-2.5 text-[11px] text-[#8A93AC]">of invoiced amount</p>
          </div>
